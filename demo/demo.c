...
HW_VOID registerBroadcast()
{
	...
	HW_BroadCastReg(IOTA_TOPIC_MQTT_DATA_PUB_RSP, Device_MqttDataPubRspHandler);
	HW_BroadCastReg(IOTA_TOPIC_MQTT_TOPIC_SUB_RSP, Device_MqttSubTopicRspHandler);
}
...
HW_INT Device_ConnectedHandler(HW_UINT uiCookie, HW_MSG pstMsg)
{
	...
	g_stGateWayInfo.pcAccessToken = HW_MsgGetStr(pstMsg, EN_IOTA_LGN_IE_ACCESSTOKEN);

	Gateway_DataReport(&pcJsonStr);
	g_uiCookie++;
	HW_Sleep(5);

	Gateway_DataReportByMqttPub();
	g_uiCookie++;
	HW_Sleep(5);
...
HW_INT Device_MqttDataPubRspHandler(HW_UINT uiCookie, HW_MSG pstMsg)
{
	HW_UINT uiResult;

	HW_LOG_INF("---------------Device_MqttDataPubRspHandler ----------------!");

	uiResult = HW_MsgGetUint(pstMsg, EN_IOTA_DATATRANS_IE_RESULT, HW_FAILED);
	if (HW_SUCCESS == uiResult)
	{
		HW_LOG_INF("--------------MQTT Data Publish Success ----------------!");
	}
	else
	{
		HW_LOG_INF("--------------MQTT Data Publish Failed ----------------!");
	}

	IOTA_SubscribeTopic(g_uiCookie, "/huawei/testtopic/#", 1);
	g_uiCookie++;
	return HW_OK;
}

HW_INT Device_MqttSubTopicRspHandler(HW_UINT uiCookie, HW_MSG pstMsg)
{
	HW_UINT uiResult;

	HW_LOG_INF("---------------Device_MqttSubTopicRspHandler ----------------!");

	uiResult = HW_MsgGetUint(pstMsg, EN_IOTA_DATATRANS_IE_RESULT, HW_FAILED);
	if (HW_SUCCESS == uiResult)
	{
		HW_LOG_INF("--------------MQTT Sub Topic Success ----------------!");
	}
	else
	{
		HW_LOG_INF("--------------MQTT Data Publish Failed ----------------!");
...
HW_VOID Gateway_DataReportByMqttPub()
{
	HW_BYTES bstrServiceData = { 0 };
	HW_JSON json;
	HW_JSON headerJson;
	HW_JSON bodyJson;
	HW_JSONOBJ hJsonObj;
	...
	headerJson = HW_JsonAddJson(json, "header");
	HW_JsonAddStr(headerJson, (HW_CHAR*)"method", (HW_CHAR*)"PUT");

	sprintf(acBuf, "/device/%s/services/Storage", g_stGateWayInfo.pcDeviceID);
	HW_JsonAddStr(headerJson, (HW_CHAR*)"from", acBuf);
	sprintf(acBuf, "/data/v1.1.0/devices/%s/services/Storage", g_stGateWayInfo.pcDeviceID);
	HW_JsonAddStr(headerJson, (HW_CHAR*)"to", acBuf);

	HW_JsonAddStr(headerJson, (HW_CHAR*)"access_token", g_stGateWayInfo.pcAccessToken);

	time(&timep);
    p=gmtime(&timep); // UTC时间
	sprintf(acBuf, "%d%02d%02dT%02d%02d%02dZ",
		p->tm_year + 1900, p->tm_mon + 1, p->tm_mday,
		p->tm_hour, p->tm_min, p->tm_sec);

	HW_JsonAddStr(headerJson, (HW_CHAR*)"timestamp", acBuf);
	HW_JsonAddStr(headerJson, (HW_CHAR*)"eventTime", acBuf);

	bodyJson = HW_JsonAddJson(json, "body");
	HW_JsonAddUint(bodyJson, (HW_CHAR*)"storage", (HW_INT)1024);
	HW_JsonAddUint(bodyJson, (HW_CHAR*)"usedPercent", (HW_INT)18);
	bstrServiceData.pcByte = HW_JsonEncodeStr(hJsonObj);
	bstrServiceData.uiLen = StrLen(bstrServiceData.pcByte);

	IOTA_MqttDataPub(g_uiCookie, "/cloud/signaltrans/v2/categories/data", 1, &bstrServiceData);
}

HW_UINT StrLen(HW_CHAR *pucStr)
{
	HW_CHAR *pucChr = pucStr;

	if (HW_NULL == pucStr) return 0;
